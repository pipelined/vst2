...
func TestPluginParameters(t *testing.T) {
	...
	testPlugin := func(fn func(p *sdk.Effect)) func(*testing.T) {
	...
	t.Run("get params", testPlugin(func(p *sdk.Effect) {
	...
	t.Run("set param", testPlugin(func(p *sdk.Effect) {
	...
	t.Run("get programs", testPlugin(func(p *sdk.Effect) {
	...
	t.Run("set program name", testPlugin(func(p *sdk.Effect) {
	...
	t.Run("set program", testPlugin(func(p *sdk.Effect) {
		assertEqual(t, "program name before", p.CurrentProgramName(), "! Startup Juno Osc TAL")
		p.SetProgram(2)
		assertEqual(t, "program name after", p.CurrentProgramName(), "ARP 303 Like FN")
	}))
	t.Run("set program data", testPlugin(func(p *sdk.Effect) {
		assertEqual(t, "resonance before", p.ParamValue(4), float32(0))
	...
	t.Run("set bank data", testPlugin(func(p *sdk.Effect) {
		assertEqual(t, "resonance before", p.ParamValue(4), float32(0))
